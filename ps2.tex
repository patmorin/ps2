...
%\todo[inline]{do we have to force the spanning tree to contain an edge of $f_0$ or we set $f_0$ after $T$ (any face can be the outer face) -- we might need to take specific face $f_0$ for the proof of theorem 8.}
...
\begin{lem}\label{reconstruction}
  ...
  Let $T$ be a binary tree, let $S\subseteq V(T)$, and let $T_0$ be the minimal subtree of $T$ that spans $S$. Then there exists an algorithm that, given an $O(1)$-query time lowest common ancestor data structure for $T$,  computes the model $T_0'$ of $T_0$ with respect to $S$ that.
\end{lem}
