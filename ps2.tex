...
\begin{proof}
    ...
    Let $x$ and $z$ be two (not necessarily distinct) vertices of $L_{h-1}$ and let $P$ be a path in $G$ of length at most $2$ whose endpoints are $x$ and $z$ and whose interior vertex (if any) is in $L_{h-1}\cup L_h$. Let $V_P$ the subset of $L_{h-1}\cup L_h$ that are on or above $P$.   We will prove, by induction on the number of vertices of $L_h$ that are on or above $P$, that there is a sequence of contractions of edges of $G_P$
    \begin{inparaenum}[(1)]
        \item that satisfies the requirements of the lemma,
        \item that does not include any edge of $P$,
        \item that eliminates all the vertices of $L_h$ that are above $P$, and
        \item such that each contracted edge that is incident to $x$ or $z$ satisfies (\ref{low_degree}).  (Contracted edges not incident to $x$ or $z$ satisfy at least one of (\ref{low_degree}) or (\ref{only_child}).)
    \end{inparaenum}

    We first observe that, if $G$ contains a separating triangle with all of its vertices on or above $P$

    \todo[inline]{first deal with separating triangles}

    If $|V(G_P)\cap L_h|=0$, then there is nothing to prove since an empty sequence of edges already satisfies the conditions of the lemma.  If $V(G_P)\cap L_{h-1}=\{x,z\}$ then the proof is rather easy because $N_G(v)\cap L_{h-1}\subseteq \{x,z\}$, so $|N_G(v)\cap L_{h-1}|\le 2$ for each $v\in V(G_P)\cap L_h$.  It is well know that any BFS layer of a planar graph induces an outerplanar graph and that any outerplanar graph with at least one vertex contains a vertex of degree at most $2$.  Therefore there exists a vertex $v\in V(G_P)\cap L_h$ with $|N_G(v)\cap L_h|\le 2$ and $|N_G(v)\cap L_{h-1}|\le 2$.  By definition, $N_G(v)$ includes some vertex $u\in L_{h-1}$  (in fact $u\in \{x,z\}$).  The edge $uv$ becomes the first edge in the contraction sequence and then the inductive hypothesis provides the rest of the sequence.  Note that the preceding discussion includes the case in which $x=z$, so we may now assume that $x\neq z$.  Furthermore, if there exists any $v\in V(G_P)\cap L_h$ such that $N_G(v)=\{x,z\}$, then $V(G_P)\cap L_{h-1}=\{x,z\}$ which is also covered by the preceding discussion. Thus, we may assume that each vertex $v\in V(G_P)\cap L_h$ has at least one neighbour $u\in L_{h-1}\setminus\{x,z\}$.
    ...
    \begin{enumerate}
        ...
        \begin{compactenum}
            \item If $x'\in L_{h-1}$, then we can apply the inductive hypothesis to find an edge contraction sequence that eliminates all vertices of $V(G_P)\cap L_h$ that are above $xx'$. We can then immediately follow this with the edge contraction sequence obtained by using the inductive hypothesis to find a contraction sequence that eliminates all vertices of $G_P\cap L_H$ that are above the edge $x'z$.  Since $xzx'$ is a face of $G_P$, this eliminates all vertices of $V(G_P)\cap L_h$ as required.

            \item If $x'\in L_h$ then let $z'$ be a neighbour of $x'$ in $L_{h-1}\setminus\{x,z\}$. Next, apply the inductive hypothesis to eliminate all vertices of $V(G_P)\cap L_h$ above the paths $P_1:=xx'z'$ and $P_2:=z'x'z$.  This leaves only one vertex, namely $x'$, of $V(G_P)\cap L_h$.  At this point, the neighbourhood of $z'$ contains only one vertex in $L_h$, namely $x'$, so the edge $z'x'$ satisfies (\ref{only_child}). We finish by contracting the edge $z'x'$.
        \end{compactenum}

        \item If $P$ has length $2$ then let $y$ denote the internal vertex of $P=xyz$.  We split this into two cases:
        \begin{compactenum}
            \item If $y\in L_{h-1}$ then we apply the inductive hypothesis to (the subgraphs of $G_P$ on or above) $P_1:=xy$ and to $P_2:=yz$.
            ...
            \item If $y\in L_h$ then there is more work to do.  In this case we consider the unique face $xyx'$ of $G$ for which $x'$ is above $xy$. Note that, if $x'=z$



            If $x'\in L_{h-1}$ then we can apply induction on the paths $P_1:=xx'$ and $P_2:=x'yz$ to obtain an edge contraction sequence that eliminates all vertices of $V(G_P)\cap L_h$ except for $y$. At this point $x$ has only one neighbour, namely $y$, in $L_{h}$, so the edge $x'y$ satisifes (\ref{only_child}).



            Furthermore, if $x'=z$ then \ldots\todo{Finish the $x'=z$ argument}  We finish by contracting the edge $x'z$.
        \end{compactenum}


        we consider the face $f$ of $G$
    \end{enumerate}
