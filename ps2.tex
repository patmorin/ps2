...
\newcommand{\qap}{\overset{\rightharpoonup}{\psi}}
\newcommand{\qbp}{\overset{\rightharpoondown}{\psi}}
...
\begin{compactenum}[(i)]
  \item If $uv_i\in E(F)$ then $\qa(uv_i)=\qb(uv_i)$ for each $i\in\{1,\ldots,d\}$.
  \item If $\depth_F(u)\neq\height(F)-1$ then $\qa(uv_1),\ldots,\qa(uv_d)$ are distinct and $\qa(uv_i)=\qb(uv_i)$ for each $i\in\{1,\ldots,d\}$.
...
\end{compactenum}
Observe that it is straightforward to create a quarter-edge labelling $(\qa,\qb)$ for $(G,F)$ by setting $\qa(uv_i):=\qb(uv_i):=i$ for each vertex $u$ of $v$ with neighbour cycle $v_1,\ldots,v_d$.
...
\begin{lem}\label{contraction_labelling}
  ...
  \begin{compactenum}[(a)]
    \item For the two faces $uvx$ and $vuy$ of $G$, $\qap(ux)=\qa(ux)$, $\qbp(ux):=c$, $\qbp(uy)=\qb(uy)$, and $\qap(uy)=c$.
    \item For each $uw\in E(G')\setminus E(G)$, $\qap(uw):=\qbp(uv):=\qa(uv)$.
    \item For each $uv\in E(G)\cap E(G')\setminus \{ux,uy\}$, $\qap(uv)=\qa(uv)$ and $\qbp(uv)=\qb(uv)$
  \end{compactenum}
\end{lem}
...
For each $i\in\{0,\ldots,r\}$, let $\overline{Y}_i:=V(G_i)\setminus V(G_{i-1})\cup E(G_i)\setminus E(G_{i-1})$.  We say that a face $f=v_0,\ldots,v_r$ of $G_p$ \emph{strongly touches} $\overline{Y}_i$ if $(V(f)\cup E(f))\cap \overline{Y}_i\neq\emptyset$.  We say that $f$ \emph{touches} $\overline{Y}_i$ if $f$ strongly touches $\overline{Y}_i$ or if $f_i:=x_0x_1x_2$ and
\begin{compactenum}
...
\begin{figure}
  \begin{center}
    \includegraphics{figs/touches}
  \end{center}
  \caption{A face $f$ of $G_p$ that touches $\overline{Y}_i$}
\end{figure}
