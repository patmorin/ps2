...
\todo[inline]{%
  Drop the BFS forest $F$ altogether, and work with a layering instead.
  Strengthen the next lemma with an alternative:  There exists a vertex $v$ of depth $h$ and degree at most $4$ ($5$ is also good enough), or there exists a vertex $u$ of depth $h-1$ with exactly one neighbour of depth $h$.   These put us into the two good cases that we know how to handle.  The former case fixes the problem pointed out in Case~2 of the proof of \cref{face_trick} and the latter case puts us in Case~1 ($v$ becomes an extension of $u$'s tripod.)
}
