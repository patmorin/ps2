...
An upward path $v_0,\ldots,v_r$ in a binary tree $T$ is \emph{non-branching} if $v_{i-1}$ is the only child of $v_i$ for each $i\in\{1,\ldots,r-1\}$.  For any binary tree $T$, the \emph{model} $T'$ of $T$ is the binary tree obtained by repeatedly replacing a maximal non-branching path $v_0,\ldots,v_r$ with the edge $v_0v_r$.

\begin{lem}\label{reconstruction}
  Given a lowest common ancestor data structure for a binary tree $T$ and a subset $S:=\{x_1,\ldots,x_d\}$ of nodes in $T$, it is possible to compute the model $T'$ of $T$ in $O(d^2)$ time.
\end{lem}

\begin{proof}
  Let $T_0$ be the minimal subtree of $T$ that contains every node in $S$.  The proof is by induction on $|S|$.  The base case $|S|=1$ is trivial, since then $S^+=S$ and $T'=T_0$ is the tree with one node.

  If $|S|\ge 2$, then the first step is to determine the root of $T_0$, which must also be the root of $T'$.  Let $S:=\{x_1,\ldots,x_d\}$.  Then the root $r$ of $T_0$ can be computed by first setting $r:=x_1$ and then repeatedly setting $r:=\lca_{T}(r,x_i)$ for each $i\in\{1,\ldots,d\}$.  Now let $r_1$ be the left child of $r$ in $T$.\footnote{If $r$ has no left child in $T$ then use the right child instead, reversing the roles of left and right in the remainder of this paragraph.}  For each $x\in S\setminus\{r\}$ there are only two possibilities for $\lca_{T}(r_1,x)$
  \begin{compactenum}
    \item If $\lca_{T}(r_1,x)=r_1$ then $x$ is contained in the subtree of $T_0$ rooted at the left child, $r_1$, of $r$.
    \item If $\lca{T}(r_1,x)=r$ then $x$ is contained in the subtree of $T_0$ rooted at the right child, $r_2$ of $r$.
  \end{compactenum}
  Therefore, using $O(d)$ lowest common ancestor queries, we can determine the root $r$ of $T'$ and partition $S\setminus\{r\}$ into sets $S_1$ and $S_2$ that define the left and right subtrees of $r$.  We can now recurse on $S_1$ an $S_2$.   This yields an algorithm whose running time is bounded by the recurrence $T(d)\le O(d)+T(d_1) + T(d_2)$, where $d_1 + d_2 = d-1$.  This recurrence resolves to $T(d)\in O(d^2)$.
\end{proof}
...
We now describe how a $(G,T)$-tripod decompositions can be obtained from a sequence of faces in $G$.  For any subgraph $f$ of $G$, we define $Y_T(f):=\bigcup_{v\in V(F)} P_T(v)$.  Let $\mathcal{F}:=f_0,\ldots,f_{r}$ be a sequence of distinct faces of $G$. Let $G_{-1}$ denote the graph with no vertices and, for each $i\in\{0,\ldots,2n-1\}$, define the graph $G_i:=\bigcup_{j=0}^i (f_j\cup Y_F(f_j))$ and let $Y_i:=V(G_i)\setminus V(G_{i-1})$.  Let $\mathcal{G_F}:=\langle G_0,\ldots,G_{r}\rangle$.  Observe that, if $G_r=G$ then $\mathcal{Y_F}:=\{Y_0,\ldots,Y_{r}\}$ is a tripod decomposition of $G$.

From the preceding definition it follows that $G_i$ is $2$-connected for each $i\in\{0,\ldots,r\}$.  For any $i\in\{0,\ldots,r\}$, consider any face $f$ of $G_i$, that we now treat as a cycle in $G$. An easy proof by induction shows that, for any $j\in\{0,\ldots,i\}$, the induced graph $f[Y_j]$ is a path.  (The path $f[Y_j]$ is a path that first appears in $G_j$.)  We are interested in keeping the number of these paths as small as possible, which motivates our next definition.
...
In this section we show that there are $O(n)$ time algorithms for computing the decompositions in \cref{meta}.b and \cref{meta}.c.
