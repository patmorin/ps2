...
In addition to planar graphs, product structure theorems similar to \cref{meta} exist for $k$-planar graphs, bounded-genus graphs, and graphs from apex-minor-free families \cite{X,Y}.  The existence of each of these decompositions relies at some point on \cref{meta}.  Thus, the algorithm presented here can also serve as an optimal subroutine in the computation of product structure decompositions of bounded-genus graphs, graph from apex-minor-free families, and $k$-planar graphs.  However, these larger graph classes also require additional machinery that (at the time of writing) makes finding these decompositions largely impractical.
