...
\begin{lem}
  Let $G$ be a triangulation with $n\ge 4$ vertices, let $f_0$ be the outer face of $G$, let $S\subseteq V(f_0)$, let $F$ be an $S$-rooted BFS forest of $G$, and let $h:=\height(F)$.  Then $F$ contains an edge $uv$ such that
  \begin{compactenum}[1.]
    \item No separating triangle of $G$ contains $uv$; \label{no_separating_triangle2}
    \item $h=\depth(v)=\depth(u)+1$; and \label{max_depth}
    \item
    \begin{compactenum}[(a)]
      \item $\deg_G(v)\le 5$, or\label{low_degree}
      \item $\deg_F(u)\le 2$.\label{only_child}
    \end{compactenum}
  \end{compactenum}
\end{lem}

\begin{proof}
  % Let $h:=\height(F)$.  Say that a separating triangle $abc$ of $G$ is \emph{worrisome} if $\{h-1,h\}=\{\depth_F(a),\depth_F(b),\depth_F(c)\}$.  If $G$ contains no worrisome separating triangles then we can ignore \cref{no_separating_triangle}.  Define a partial order $\prec$ on the separating triangles of $G$ in which $\Delta_1\prec \Delta_2$ if the interior of $\Delta_1$ is contained in the interior of $\Delta_2$.  Now consider any worrisome separating triangle $f_0'$ that is minimal with respect to $\prec$.  Let $G'$ be the subgraph of $G$ induced by the vertices of $f_0'$ and the vertices in the interior of $f_0'$.  Then $G'$ has $n'\ge 4$ vertices, outer face $f'$, and $F':=F\cap G'$ is an $S'$-rooted spanning forest of $G'$ for some $S'\subseteq f_0'$.  Therefore it suffices to prove the result for $G'$, which has no worrisome separating triangles.  We therefore assume that $G$ has no worrisome separating triangles, so we can ignore the first requirement.
  
  Let us ignore the requirement \cref{no_separating_triangle2} for now.  Let $L_0:=\{u\in V(G):\depth_F(u)=h-1\}$ and let $L_1:=\{v\in V(G):\depth_F(v)=h\}$.
...
For each $i\in\{0,\ldots,2n-3\}$, let $G[i]:=G[\bigcup_{j=0}^i\tau_j]$. Note that $G_i$ contains $\bigcup_{j=0}^i \tau_j$ as a subgraph.  For each vertex $v$ of $G$, let $j_\mathcal{F}(v):=\min\{j:v\in V(G_j)$ and for each edge $e$ of $G$, let $j_\mathcal{F}(e):=\min\{j:v\in E(G_j)\}$.
...
\begin{proof}[Proof of \cref{face_trick}]
  ...
  \begin{enumerate}
    ...
    \todo[inline]{This is now broken.  The problem occurs at the first $s>r$ such that $f_s$ is incident on $v$.  Essentially, the face $f_s'$ was chosen because it gives a tripd $\tau_s'$ that has feet on $\tau_i'$, $\tau_r'$ and $\tau_q'$ for some $q$.  But now $\tau_s$ has two feet on $\tau_r$ and no feet on $\tau_i$.  This seems to fuck everything up.  The only obvious observation here is that $f_s$ should be one of the triangles incident on $v$, which could help if we can guarantee that $v$ has low degree.  The problem seems to go away if $v$ has degree at most $5$, and looks}
