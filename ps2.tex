...
In this section we present a new proof of (a strengthening of ) \cref{face_trick}.  We first explain why a naïve attempt at an inductive proof using edge contractions does not work. Consider a proof with the following structure:
\begin{compactenum}
...
A \emph{quarter-edge labelling} $(\qa,\qb)$ of $(G,F)$ is a pair of functions  defined over the directed edges of $G$ that satisfy the following properties for each vertex $u$ of $G$ with neighbour cycle\todo{Define \emph{neighbour cycle}} $v_1,\ldots,v_d$ where $v_1\prec_F u$:
\begin{compactenum}[(i)]
  \item If $uv_i\in E(F)$ or $\depth_F(v_i)<\depth_F(u)$ or $\depth_F(u)<\height(F)-1$ then $\qa(uv_i)=\qb(uv_i)=c$ and $c\not\in \{\qa(uv_i),\qb(uv_i)$ for any $i\in\{1,\ldots,d\}\setminus\{i\}$.
  % \item If $\depth_F(u)<\height(F)-1$ then $\qa(uv_1),\ldots,\qa(uv_d)$ are distinct and $\qa(uv_i)=\qb(uv_i)$ for each $i\in\{1,\ldots,d\}$.
  \item For each $i\in\{1,\ldots,d-1\}$: If $\qa(uv_i)=\qa(uv_{i+1})$ then $\qb(uv_{i+1})=\qa(uv_i)$.\todo{Modify to ensure no gaps}
  \item For each $i\in\{1,\ldots,d-1\}$: If $\qb(u_i)=\qb(uv_{i+1})$ then $\qa(uv_{i})=\qb(uv_i)$.
\end{compactenum}
Observe that it is straightforward to create a quarter-edge labelling $(\qa,\qb)$ for $(G,F)$ by setting $\qa(uv_i):=\qb(uv_i):=i$ for each vertex $u$ of $v$ with neighbour cycle $v_1,\ldots,v_d$.  The next lemma, illustrated in \cref{contracting} shows how to update a quarter-edge labelling during an edge contraction.
...
\begin{compactenum}
  \item there exists $i\in\{0,1,2\}$ and $j\in\{0,\ldots,r\}$ such that  $x_i=v_j$ and $\qa(x_ix_{i-1})=\qa(v_jv_{j+1})$; or
  \item there exists $i\in\{0,1,2\}$ and $j\in\{0,\ldots,r\}$ such that  $x_i=v_j$ and $\qb(x_ix_{i+1})=\qb(v_jv_{j-1})$.
\end{compactenum}
...
\begin{proof}
  ...
  By construction $\mathcal{F}$ contains every face of $G$.  All that remains is to verify that, for each $i\in\{0,\ldots,r\}$ and each face $f$ of $G[i]$, there are at most three tripods among $Y_0,\ldots,Y_i$ that touch $f$ with respect to $\qa$ and $\qb$, which is what we now show.  Let $c:=\qa(uv)$. Since $uv\in E(F)$, $\qb(uv)=c$.  Let $f:=v_0,\ldots,v_r$.  We distinguish between some cases:

  \begin{enumerate}
    \item $v\not\in\{v_0,\ldots,v_r\}$. In this case, $f$ also a face of $G_i'$ and, by induction, $f$ touches at most three tripods among $Y_0',\ldots,Y_i'$.  If  $\qa(e)=\qap(e)$ and $\qb(e)=\qbp(e)$ for each edge $e\in E(f)$ then $f$ touches $Y_j$ with respect to $(\qa,\qb)$ for some $j\in\{0,\ldots,i\}$ if and only if $f$ touches $Y_j'$ with respect to $(\qap,\qbp)$.  Thus we need only consider the case where $u=v_i$ for some $i\in\{0,\ldots,t\}$ and $\qap(uv_{i+1})=c$ or $\qbp(uv_{i-1})=c$.  We claim that this case does not occur.  Indeed, if (say) $\qap(uv_{i+1})=c$ then $uv_{i+1}\not\in E(G)$.  But since $uv_{i+1}\in E(f)$, this implies that that $f$ contains $v$, contradicting the assumption that leads to this case.

    \item $u=v_i$ and $v=v_{i+1}$ for some $i\in\{0,\ldots,t\}$.\footnote{There is a symmetric case in which $v=v_i$ and $u=v_{i+1}$.} Suppose $t>2$ so that $f$ has four or more edges.\todo{Deal with the $t=2$ case later.}  Then the face $f':=v_0,\ldots,v_{i},v_{i+2},\ldots,v_t$ is a face of $G_i'$ and $\qap(uv_{i+2})=c$.  Therefore, if $Y_j$ is the tripod that contains $v$ then $f'$ touches $Y_j'$.  Suppose that $f$ touches some other tripod $Y_a\neq Y_j$ and $f'$.  If $f$ strongly touches $Y_a$ then $f'$ strongly touches $Y_a'$ so we need only consider the case where $f$ weakly touches $Y_a$.   This happens because $f_a:=vxy$ and $\qa(vy)=\qa(vv_{i+2})$.  If $uv_{i+2}\not\in E(G)$ then this implies that $\qa(uv_{i+1})=c$ so $f$ weakly touches $Y_a$ and we are done.  If $uv_{i+1}\in E(G)$ then either $uvv_{i+2}$ is a separating triangle or $f=uvv_{i+2}$ and $t=2$.  Either case is a contradiction since $uv$ is contractible and $t>2$.

    \item $u\not\in V(f)$ and $v=v_i$ for some $i\in\{0,\ldots,t\}$.
  \end{enumerate}
\end{proof}
