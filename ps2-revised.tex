...
\begin{proof}
  ...
  Therefore $X$ is a subgraph of $\overline{T}$ that has a component containing all the elements of $S$. Therefore $X$ contains $\overline{T}_0$.  By choice, $\overline{T}_0$ contains a path from $f_i$ to each $g\in S$ and each of these paths is disjoint except for their shared starting point $f_i$.   
...
\begin{enumerate}[nosep,nolistsep]
  ...
  %\item A non-empty subproblem is bichromatic if $uv$ if bichromatic. The input to a bichromatic subproblem is the half-edge $e_1'=vu$ and the half-edge $e_2'=e_2$.
...
Trichromatic instances induce at most three recursive invocations. We must use point 1 above to identify subproblems. Let $uvw$ be the vertices of $f_i$. Then, there exits a subproblem to the right of $uv$, a subproblem to the right of $vw$, and a subproblem to the right of $wu$.  Without loss of generality, suppose that the subproblem to the right of $uv$ contains $e_1$, the suproblem to the right of $vw$ contains $e_2$ and the subproblem to the right of $wu$ contains $e_3$.
